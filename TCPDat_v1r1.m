...
season = double(netcdf.getVar(ncid,3)); % Year based on season (01 January
...
nature = double(netcdf.getVar(ncid,17)); % Storm nature: 0 = TS - tropical (i.e. hurricane,
...
name = netcdf.getVar(ncid,1); % Storm name. All storms were unnamed prior to
...
numObs = double(netcdf.getVar(ncid,2)); % Number of observations for the 
...
track_type = double(netcdf.getVar(ncid,4)); % 0 = main (cyclogenesis (cg)
...
basin = double(netcdf.getVar(ncid,7)); % Based on present location. 0 = NA,
% 12 = CS, 13 = GM -127 = NaN
...
dist2land = double(netcdf.getVar(ncid,19)); % Distance to land 
% Units: km -999 = NaN
dist2land = dist2land(:,start_season:end_season);
%dist2land = dist2land(:,s48:e15);
dist2land(dist2land == -999) = NaN;

landfall = double(netcdf.getVar(ncid,20)); % Minimum distance to land until
% next report (0=landfall) units: km -999 = NaN
landfall = landfall(:,start_season:end_season);
%landfall = landfall(:,s48:e15);
landfall(landfall == -999) = NaN;
...
for i = 1:naty % replace with column variable for nature
    for j = 1:natx % same thing, but with rows
        if (nature(j,i) > 0) && (nature(j,i) < 8) % This just focuses on the 
...
for b = 1:pdl% 58 is 2005 youcould also use the length of yr_uniq.
    ...
    elseif yr_uniq(b) == 2015 % 2015 is strange and has to be singled out.
