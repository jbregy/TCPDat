...
% Date updated: 10/02/2023 (dd/mm/yyyy)
...
% frequently checked email is jbregy@clemson.edu, but I also have a stable
...
% CONUS_daily_precip. (I previously had it as CONUS_daily_precip_1948_2016
% because I accidentally downloaded p2016.nc, but HURDAT2 had not been updated
% to include 2016.) I will be changing the file name in the command to 
% CONUS_daily_precip, so I suggest that you create a file with the same name. 
% That, or change the name in the code yourself. Alternatively, you could just 
% comment out directory changes that are used to access data as long as the 
% folder containing your precipitation (and I suppose IBTrACS, too) is included 
% whenever you set your path.
...
% you see fit.
year = season(start_season:end_season,:); % Extracts the years of interest. For
...
varid = netcdf.inqVarID(ncid,'usa_lat');%'lat');
...
varid = netcdf.inqVarID(ncid,'usa_lon');%'lon');
...
varid = netcdf.inqVarID(ncid,'usa_roci'); % This is the radius of the outermost closed isobar. It will be used to extract the TC rainfield. 
roci = double(netcdf.getVar(ncid,varid));
roci = roci(:,start_season:end_season);
roci(roci == -9999) = NaN;
roci = nm2km(roci);

varid = netcdf.inqVarID(ncid,'usa_rmw');
rmw = double(netcdf.getVar(ncid,varid));
rmw = rmw(:,start_season:end_season);
rmw(rmw <= -9999) = NaN;
rmw = nm2km(rmw);
...
% For future iterations, we might go ahead and just switch the landfall
% distance to 500 km since we're using a variable rainshield radius.
count = 0; % this and the for-if loop determines the number of storms within 
% the specified distance from land.
user_distance = input('What maximum distance from land would you like to use?') % This was originally used to define the rainfield radius but now I'm trying to use mean ROCI to define the rainfield.
for i = 1:naty
...
cd CONUS_daily_precip\%_1948_2016\ % This line is user specific. 
% cd CONUS_daily_precip\ 
pd = dir('precip.V1.0.*.nc');%'p*.nc');
pdl = length(pd);
cd ../ 
...
for b = 1:pdl% 58 is 2005 youcould also use the length of yr_uniq.
    ...
    elseif yr_uniq(b) == 2015 %| yr_uniq(b) == 2018 | yr_uniq(b) == 2019% 2015 is strange and has to be singled out.
        ...
        tcyroi_loc_en = find(tcyear(1,:) == yr_uniq(1,b),1,'last');
    elseif yr_uniq(b) == 2018
        [~, tcyroi_loc_st] = find(tcyear == yr_uniq(1,b),1,'first');
        [~, tcyroi_loc_en] = find(tcyear == yr_uniq(1,b),1,'last');
        tcyroi = tcyear(:,tcyroi_loc_st:tcyroi_loc_en);
    ...
    elseif yr_uniq(b) == 1955
        ...
        % similar to 2015, but I will update that in future versions.
        [~, tcyroi_loc_st] = find(tcyear == yr_uniq(1,b),1,'first');
        [~, tcyroi_loc_en] = find(tcyear == yr_uniq(1,b),1,'last');
        if yr_uniq(1,b) == 2014
            tcyroi_loc_en = tcyroi_loc_en-1; % Not pretty, but I'm tired and hungry.
        else
        end
        tcyroi = tcyear(:,tcyroi_loc_st:tcyroi_loc_en);
%         tcyroi = tcyear(:,(find(tcyear(1,:) == yr_uniq(1,b),1,'first'):find(tcyear(1,:) == yr_uniq(1,b),1,'last')));
...
%         else
   ...
   % tcbasin_oi = tcbasin(:,tcyroi_loc_st:tcyroi_loc_en);
    ...
    tcroci_oi = tcroci(:,tcyroi_loc_st:tcyroi_loc_en);
  % COME BACK AND FIGURE OUT THE ROCI PROBLEM. IT SEEMS LIKE ROCI ONLY
  % EXISTS FROM 2001 ONWARD, WHICH COULD BE PROBLEMATIC AS IT MIGHT PRODUCE
  % AN ARTIFICIAL TREND.
    
    % Opening URD files and extracting data.
    cd CONUS_daily_precip\ %_1948_2016\ 
    ...
    for a = 1:oiy % essentially the number of storms based on the length of the year % 9 is Katrina in 2005
        ...
        elseif any(nancheck == 0) %== 0 % add the logical false (0) if there are any NaNs.
            interruptions = 1;
            prenan_loc = find(nancheck == 1,1,'first')-1;%find(isnan(tclon_oi(:,a)),1,'first')-1;
            postnan_loc = find(nancheck == 1,1,'last')+1;%find(isnan(tclon_oi(:,a)),1,'last')+1;
            x = tclon_oi(svl1:prenan_loc,a)';
            x2 = tclon_oi(postnan_loc:evl1,a)';
            y = tclat_oi(svl1:prenan_loc,a)';
            y2 = tclat_oi(postnan_loc:evl1,a)';
            t = [0, cumsum(abs(diff(x))+abs(diff(y)))];
            t2 = [0, cumsum(abs(diff(x2))+abs(diff(y2)),'omitnan')];
            if length(x) == length(unique(t)) && length(y) == length(unique(t))
                npts=2*length(x);
            ...
            if length(x2) == length(unique(t2)) && length(y2) == length(unique(t2))
                ...
                elseif length(x2) ~= length(unique(t2)) && length(y2) ~= length(unique(t2))
                    ...
                    xi2 = spline(unique(t2),x2,ti2); xi2 = xi2';
                    yi2 = spline(unique(t2),y2,ti2); yi2 = yi2';
            ...
            for hr2 = 1:length(hurhour_utc2(find(hurhour_utc2 >= 0,1,'first'):find(hurhour_utc2 >= 0,1,'last')))
        ...
        if interruptions == 0
            for k = 1:hdoyux
                ...
                tc_boosum = sum(tc_boo,3,'omitnan'); %nansum(tc_boo,3); % sum up all of the presence/absence 
            ...
            for k2 = 1:hdoyux2
                ...
                doh_loc2 = (find(hur_doy2(find(hur_doy2 > 0,1,'first'):find(hur_doy2 > 0,1,'last')) == hdoy_uniq2(k2),1,'first'):find(hur_doy2(find(hur_doy2 > 0,1,'first'):find(hur_doy2 > 0,1,'last')) == hdoy_uniq2(k2),1,'last'));
                [~, checkdohly2] = size(doh_loc2);
                if checkdohly2 < k2
                    [~, dohly2] = size(doh_loc2);
                else
                    [~ , dohly2] = size(doh_loc2(1:k2)); % switch ~ with dohlx2 if you mess up
                end
                %[~ , dohly2] = size(doh_loc2(1:k2)); % switch ~ with dohlx2 if you mess up
                tc_boo2 = zeros(plonx,platx,dohly2);
                mm2 = 1;
                for m2 = doh_loc2(1):doh_loc2(dohly2)
                    if m2 > length(xi2) || m2 > length(yi2)
                        rxi2 = repmat(xi2(m2-1,1),300,120);
                        ryi2 = repmat(yi2(m2-1,1),300,120);
                    else
...
%                     ryi2 = repmat(yi2(m2,1),300,120);
                ...
                tc_boosum2 = sum(tc_boo2,3,'omitnan');%nansum(tc_boo2,3); % sum up all of the 
                ...
                % for the day
                hdoy_grid_boo2(:,:,k2) = tc_boosum2; % puts tc_boosum 
    ...
    cd TCP_data_netcdf\%_test\
    ncid_tcp_daily = netcdf.create(['TCP_daily_',num2str(b+(st_yr-1),4),'.nc'],'NC_WRITE');
    dimidlat_daily = netcdf.defDim(ncid_tcp_daily,'lat',120);
    dimidlon_daily = netcdf.defDim(ncid_tcp_daily,'lon',300);
    dimidt_daily = netcdf.defDim(ncid_tcp_daily,'time',length(pday_calcount));
    
    latitude_ID_daily = netcdf.defVar(ncid_tcp_daily,'lat','NC_FLOAT',dimidlat_daily);
    longitude_ID_daily = netcdf.defVar(ncid_tcp_daily,'lon','NC_FLOAT',dimidlon_daily);
    ...
    netcdf.reDef(ncid_tcp_daily);
    netcdf.putAtt(ncid_tcp_daily,0,'units','degrees_north');
    netcdf.putAtt(ncid_tcp_daily,0,'long_name','Latitude');
    netcdf.putAtt(ncid_tcp_daily,0,'actual_range',[20.1250 49.8750]);
    netcdf.putAtt(ncid_tcp_daily,0,'standard_name','latitude');
    netcdf.putAtt(ncid_tcp_daily,0,'axis','Y');
    
    netcdf.putAtt(ncid_tcp_daily,1,'units','decimal_degrees');
    netcdf.putAtt(ncid_tcp_daily,1,'units_note','Add 360 for degrees_east');
    netcdf.putAtt(ncid_tcp_daily,1,'long_name','Longitude');
    netcdf.putAtt(ncid_tcp_daily,1,'actual_range',[-55.1250 -129.8750]);
    netcdf.putAtt(ncid_tcp_daily,1,'standard_name','longitude');
    netcdf.putAtt(ncid_tcp_daily,1,'axis','X');
    
    netcdf.putAtt(ncid_tcp_daily,2,'units','day');
    netcdf.putAtt(ncid_tcp_daily,2,'long_name','Gregorian day of the year');
    netcdf.putAtt(ncid_tcp_daily,2,'valid_range',[1 366]);
    if leapyear(yr_uniq(b))
        netcdf.putAtt(ncid_tcp_daily,2,'actual_range',[1 366]);
    else
        netcdf.putAtt(ncid_tcp_daily,2,'actual_range',[1 365]);
    end
    netcdf.putAtt(ncid_tcp_daily,2,'standard_name','DoY');
    
    netcdf.putAtt(ncid_tcp_daily,3,'units','mm');
    netcdf.putAtt(ncid_tcp_daily,3,'long_name','Daily accumulated TCP');
    netcdf.putAtt(ncid_tcp_daily,3,'range',[0 max(TCP_daily_grid,[],'all','omitnan')]);
    netcdf.putAtt(ncid_tcp_daily,3,'level_desc','surface');
    netcdf.putAtt(ncid_tcp_daily,3,'missing_value','NaN');
    netcdf.endDef(ncid_tcp_daily);
...
% Define dimensions
dimidlat_ann = netcdf.defDim(ncid_tcp_ann,'lat',120); % This defines latitude. It should always be 120.
dimidlon_ann = netcdf.defDim(ncid_tcp_ann,'lon',300); % This defines longitude. It should always be 300.
...
latitude_ID_ann = netcdf.defVar(ncid_tcp_ann,'lat','NC_FLOAT',dimidlat_ann); % Single datatype is used for lat and lon just because that's what URD uses.
longitude_ID_ann = netcdf.defVar(ncid_tcp_ann,'lon','NC_FLOAT',dimidlon_ann);
...
netcdf.reDef(ncid_tcp_ann);
netcdf.putAtt(ncid_tcp_ann,0,'units','degrees_north');
netcdf.putAtt(ncid_tcp_ann,0,'long_name','Latitude');
netcdf.putAtt(ncid_tcp_ann,0,'actual_range',[20.1250 49.8750]);
netcdf.putAtt(ncid_tcp_ann,0,'standard_name','latitude');
netcdf.putAtt(ncid_tcp_ann,0,'axis','Y');

netcdf.putAtt(ncid_tcp_ann,1,'units','decimal_degrees');
netcdf.putAtt(ncid_tcp_ann,1,'units_note','Add 360 for degrees_east');
netcdf.putAtt(ncid_tcp_ann,1,'long_name','Longitude');
netcdf.putAtt(ncid_tcp_ann,1,'actual_range',[-55.1250 -129.8750]);
netcdf.putAtt(ncid_tcp_ann,1,'standard_name','longitude');
netcdf.putAtt(ncid_tcp_ann,1,'axis','X');

netcdf.putAtt(ncid_tcp_ann,2,'units','year');
netcdf.putAtt(ncid_tcp_ann,2,'long_name','Year');
netcdf.putAtt(ncid_tcp_ann,2,'actual_range',[1948 max(yr_uniq)]);

netcdf.putAtt(ncid_tcp_ann,3,'units','mm');
netcdf.putAtt(ncid_tcp_ann,3,'long_name','Total Annual TCP');
netcdf.putAtt(ncid_tcp_ann,3,'range',[0 max(TCP_annual,[],'all','omitnan')]);
netcdf.putAtt(ncid_tcp_ann,3,'level_desc','surface');
netcdf.putAtt(ncid_tcp_ann,3,'missing_value','NaN');
netcdf.endDef(ncid_tcp_ann);

netcdf.close(ncid_tcp_ann);
cd ../../

%% North Atlantic Hurricane Season TCP

% This section will extract daily TCP during the North Atlantic hurricane
% season (June–November) for each year. I added this in because it is
% useful for my paleohurricane reconstructions, though others outside of
% paleotempestology might find this useful.

% IMPORTANT: IF YOU WANT TO EXTRACT THE DATA YOURSELF, YOU WILL NEED THE
% AEROSPACE TOOLBOX INSTALLED AS THIS SECTION OF THE CODE USES THE leapyear
% FUNCTION. FOR MORE INFORMATION ABOUT THE AEROSPACE TOOLBOX, PLEASE VISIT
% THE FOLLOWING ADDRESS:
% https://www.mathworks.com/products/aerospace-toolbox.html
% FOR MORE INFORMATION ABOUT THE leapyear FUNCTION, PLEASE VISIT THE
% FOLLOWING ADDRESS: https://www.mathworks.com/help/aerotbx/ug/leapyear.html


%yr_uniq_leapyear = leapyear(yr_uniq);

%cd TCP_Data\
% cd CONUS_daily_precip\ 
pd = dir('TCP_daily_*.mat');%dir('precip.V1.0.*.nc');%'p*.nc');
pdl = length(pd);

TCP_JJASON_daily = zeros(300,120,length(yr_uniq),183); % 183 days from June 1 to November 30. 
for i = 1:pdl
    load(pd(i).name);
    if leapyear(yr_uniq(i)) == 1 % If it's a leap year
        TCP_JJASON_daily(:,:,i,:) = TCP_daily_grid(:,:,153:335); % June 1 and November 30 are the 153 and 335 day of a leap year
    else % If it's not a leap year
        TCP_JJASON_daily(:,:,i,:) = TCP_daily_grid(:,:,152:334); % Obviously just shifting it by -1
    end
end

%% netCDF of JJASON daily TCP


ncid_tcp_JJASON = netcdf.create('TCP_JJASON_daily.nc','NC_WRITE');
dimidlat_JJASON = netcdf.defDim(ncid_tcp_JJASON,'lat',120);
dimidlon_JJASON = netcdf.defDim(ncid_tcp_JJASON,'lon',300);
dimityr_JJASON = netcdf.defDim(ncid_tcp_JJASON,'year',length(yr_uniq));
dimitday_JJASON = netcdf.defDim(ncid_tcp_JJASON,'day',183); % 183 days in JJASON
latitude_ID_JJASON = netcdf.defVar(ncid_tcp_JJASON,'lat','NC_FLOAT',dimidlat_JJASON);
longitude_ID_JJASON = netcdf.defVar(ncid_tcp_JJASON,'lon','NC_FLOAT',dimidlon_JJASON);
year_ID_JJASON = netcdf.defVar(ncid_tcp_JJASON,'year','NC_DOUBLE',dimityr_JJASON);
day_ID_JJASON = netcdf.defVar(ncid_tcp_JJASON,'day','NC_DOUBLE',dimitday_JJASON);
TCPDat_JJASON_ID = netcdf.defVar(ncid_tcp_JJASON,'TCP_JJASON_daily','NC_FLOAT',[dimidlon_JJASON,dimidlat_JJASON,dimityr_JJASON,dimitday_JJASON]);
netcdf.endDef(ncid_tcp_JJASON);
netcdf.putVar(ncid_tcp_JJASON,longitude_ID_JJASON,plon);
netcdf.putVar(ncid_tcp_JJASON,latitude_ID_JJASON,plat);
netcdf.putVar(ncid_tcp_JJASON,year_ID_JJASON,yr_uniq');
JJASON_day = (1:1:183)';
netcdf.putVar(ncid_tcp_JJASON,day_ID_JJASON,JJASON_day);
netcdf.putVar(ncid_tcp_JJASON,TCPDat_JJASON_ID,TCP_JJASON_daily);

netcdf.reDef(ncid_tcp_JJASON);

netcdf.putAtt(ncid_tcp_JJASON,0,'units','degrees_north');
netcdf.putAtt(ncid_tcp_JJASON,0,'long_name','Latitude');
netcdf.putAtt(ncid_tcp_JJASON,0,'actual_range',[20.1250 49.8750]);
netcdf.putAtt(ncid_tcp_JJASON,0,'standard_name','latitude');
netcdf.putAtt(ncid_tcp_JJASON,0,'axis','Y');

netcdf.putAtt(ncid_tcp_JJASON,1,'units','decimal_degrees');
netcdf.putAtt(ncid_tcp_JJASON,1,'units_note','add 360 for degrees east');
netcdf.putAtt(ncid_tcp_JJASON,1,'long_name','Longitude');
netcdf.putAtt(ncid_tcp_JJASON,1,'actual_range',[-55.1250 -129.8750]);
netcdf.putAtt(ncid_tcp_JJASON,1,'standard_name','longitude');
netcdf.putAtt(ncid_tcp_JJASON,1,'axis','X');

netcdf.putAtt(ncid_tcp_JJASON,2,'units','year');
netcdf.putAtt(ncid_tcp_JJASON,2,'long_name','Year');
netcdf.putAtt(ncid_tcp_JJASON,2,'actual_range',[1948 2019]);
netcdf.putAtt(ncid_tcp_JJASON,2,'standard_name','year');

netcdf.putAtt(ncid_tcp_JJASON,3,'units','day');
netcdf.putAtt(ncid_tcp_JJASON,3,'long_name','Day of JJASON');
netcdf.putAtt(ncid_tcp_JJASON,3,'actual_range',[1 183]);
netcdf.putAtt(ncid_tcp_JJASON,3,'standard_name','day');

netcdf.putAtt(ncid_tcp_JJASON,4,'units','mm');
netcdf.putAtt(ncid_tcp_JJASON,4,'long_name','Daily Accumulated TCP for JJASON using 223-km search radius');
netcdf.putAtt(ncid_tcp_JJASON,4,'actual_range',[0 453.6892]);
netcdf.putAtt(ncid_tcp_JJASON,4,'valid_range',[0 500]);
netcdf.putAtt(ncid_tcp_JJASON,4,'Dataset','TCPDat');
netcdf.putAtt(ncid_tcp_JJASON,4,'level_desc','surface');
netcdf.putAtt(ncid_tcp_JJASON,4,'missing_value','NaN');

netcdf.endDef(ncid_tcp_JJASON);
netcdf.close(ncid_tcp_JJASON);